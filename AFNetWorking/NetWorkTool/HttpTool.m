...
{
    //加密后的参数字典
    NSMutableDictionary *dict=[self AddEncrypd:parameters];

    //读取缓存
    responseCache ? responseCache([HTTPCache getHttpCacheForKey:URL]) : nil;
    
    AFHTTPSessionManager *manager = [self createAFHTTPSessionManager];
    return [manager GET:URL parameters:dict progress:^(NSProgress * _Nonnull uploadProgress) {
...
//    NSDictionary* dict = [self convertToDict];
    ...
    NSString *strUrl4=[SecretKey stringByAppendingString:strUrl3];

    
    NSString *strURL5=[[strUrl4 stringByAppendingString:SecretKey] lowercaseString];
    
    return strURL5;
}
+(NSMutableDictionary*)AddEncrypd:(NSMutableDictionary*)parametersDic
{
//    parametersDic[@"appkey"]=BQAPPKey;
    NSDate *date=[NSDate date];
     NSDateFormatter *formatter=[[NSDateFormatter alloc]init];
     [formatter setDateFormat:@"yyyy-MM-dd HH:mm:ss"];
    NSLog(@"%@",[formatter stringFromDate:date]);
    parametersDic[@"ts"]=[formatter stringFromDate:date];
    parametersDic[@"sign"]= [MD5Tool MD5FromString:[self GetSign:parametersDic]];
...
+ (AFHTTPSessionManager *)createAFHTTPSessionManager
{
    ...
    //解决报错3840的问题，但是好像没有什么用，估计是后台接口的问题
                            ...
                            failure:(HttpRequestFailed)failure
{
    ...
    return [manager POST:URL parameters:dict constructingBodyWithBlock:^(id<AFMultipartFormData>  _Nonnull formData) {
        ...
        //压缩-添加-上传图片
        [imageModels enumerateObjectsUsingBlock:^(PhotoModel * _Nonnull model, NSUInteger idx, BOOL * _Nonnull stop) {
           ...
           // NSData *imageData = UIImageJPEGRepresentation(model.image, 0.5);
            ...
            /*
             ...
             4. 上传文件的[mimeType]
             */
            [formData appendPartWithFileData:model.imageData name:model.imageName fileName:model.fileName mimeType:model.mimeType];
        }];
    } progress:^(NSProgress * _Nonnull uploadProgress) {
        //上传进度
        progress ? progress(uploadProgress) : nil;
